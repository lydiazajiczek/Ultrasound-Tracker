...
Usage [optional]:
	...
	PacketClient [ServerIP] [LocalIP] [COM Port Number]
...
void Unpack(char* pData)
{
	...
	char *ptr_out = comBuff; //ptr_out will also be number of bytes to write! yay!
	...
	DWORD dwBytesWritten = 0;
	//if (!WriteFile(serialHandle, comBuff, sizeof(comBuff), &dwBytesWritten, NULL)) {
	//	std::cerr << "Error! Could not write to COM1 :(" << std::endl;
	//}
    ...
    if(MessageID == 7)      // FRAME OF MOCAP DATA packet
    {
		...
		//std::cout << std::hex << frameNumber << std::endl;
		memcpy(ptr_out, ptr, 4); //frame number
		ptr += 4;
		ptr_out += 4;
        //printf("Frame # : %d\n", frameNumber);
		...
		memcpy(ptr_out, ptr, 4); //number of rigid bodies
		ptr += 4;
		ptr_out += 4;
        ...
        for (int j=0; j < nRigidBodies; j++)
        {
			...
			memcpy(ptr_out, ptr, 4); //ID
			ptr += 4;
			ptr_out += 4;
            float x = 0.0f; memcpy(&x, ptr, 4); 
			memcpy(ptr_out, ptr, 4); //x
			ptr += 4;
			ptr_out += 4;
            float y = 0.0f; memcpy(&y, ptr, 4); 
			memcpy(ptr_out, ptr, 4); //y
			ptr += 4;
			ptr_out += 4;
            float z = 0.0f; memcpy(&z, ptr, 4); 
			memcpy(ptr_out, ptr, 4); //z
			ptr += 4;
			ptr_out += 4;
            float qx = 0; memcpy(&qx, ptr, 4); 
			memcpy(ptr_out, ptr, 4); //qx
			ptr += 4;
			ptr_out += 4;
            float qy = 0; memcpy(&qy, ptr, 4); 
			memcpy(ptr_out, ptr, 4); //qy
			ptr += 4;
			ptr_out += 4;
            float qz = 0; memcpy(&qz, ptr, 4); 
			memcpy(ptr_out, ptr, 4); //qz
			ptr += 4;
			ptr_out += 4;
            float qw = 0; memcpy(&qw, ptr, 4); 
			memcpy(ptr_out, ptr, 4); //qw
			ptr += 4;
			ptr_out += 4;
            //printf("ID : %d\n", ID);
            ...
            if( ((major == 2)&&(minor >= 6)) || (major > 2) || (major == 0) ) 
            {
				...
				memcpy(ptr_out, ptr, 2); //rigid body successfully tracked (boolean)
				ptr += 2;
				ptr_out += 2;
        ...
        if( ((major == 2)&&(minor>=7)) || (major>2))
        {
			...
			memcpy(ptr_out, ptr, 8); //timestamp (double ptr_out)
			ptr += 8;
			ptr_out += 8;
        }
        else
